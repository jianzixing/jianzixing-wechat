...
package com.jianzixing.webapp.valcode;
...
public class PixelUtils {
    ...
    public static int mix(int p1, int p2, double alpha) {
        alpha = 1 - alpha;
        PixelUtils.RGBA rgba1 = alpha(p1);
        PixelUtils.RGBA rgba2 = alpha(p2);

        int r = (int) (rgba1.red * alpha + (1 - alpha) * rgba2.red);
        int g = (int) (rgba1.green * alpha + (1 - alpha) * rgba2.green);
        int b = (int) (rgba1.blue * alpha + (1 - alpha) * rgba2.blue);
        Color color = new Color(r, g, b);
        return color.getRGB();
    }

    public static PixelUtils.RGBA alpha(int pixel) {
        int alpha = (pixel >> 24) & 0xff;
        int red = (pixel >> 16) & 0xff;
        int green = (pixel >> 8) & 0xff;
        int blue = (pixel >> 0) & 0xff;
        PixelUtils.RGBA rgba = new PixelUtils.RGBA();
        rgba.alpha = alpha;
        rgba.red = red;
        rgba.green = green;
        rgba.blue = blue;
    ...
    public static int getAverageColor(int... cs) {
        ...
        Color color = new Color((int) dr / cs.length, (int) dg / cs.length, (int) db / cs.length);
        return color.getRGB();
    }

    public static int colorReset(int c) {
        RGBA rgba = alpha(c);
        Color color = new Color(255 - rgba.red, 255 - rgba.green, 255 - rgba.blue);
        return color.getRGB();
    }
