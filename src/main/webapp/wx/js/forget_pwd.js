    ...
    $('.mesg-code').click(function () {
        if (ms >= 120) {
            ...
            if (phone && phone.length >= 11) {
                CommentUtils.wait();
                $.ajax({
                    url: 'forget_pwd_sms.jhtml', data: {phone: phone}, success: function (resp) {
    ...
    $('#loginBtn').click(function () {
        ...
        if (opacity == '1') {
            ...
            if (phone && phone.length >= 11 && password && code) {
                CommentUtils.wait();
                $.ajax({
                    url: 'forget_pwd_submit.jhtml',
                    data: {phone: phone, password: password, code: code},
                    success: function (resp) {
                        CommentUtils.closeWait();
                        if (resp == 'ok') {
                            CommentUtils.confirm('修改成功，是否马上登录？', function () {
                        ...
                        } else if (resp == 'user_not_exit') {
                            CommentUtils.confirm('该用户不存在，是否立即注册？', function () {
