Ext.define('App.discount.DiscountGoodsPanel', {
    ...
    columns: [
        ...
        {
            xtype: 'gridcolumn',
            dataIndex: 'discountSku',
            width: 180,
            text: '规格&排除',
            renderer: function (value, metaData, record, rowIndex, colIndex, store, view) {
                var discountSkuId = record.get('discountSkuId');
            ...
            var win = Dialog.openWindow('App.goods.SelectGoodsWindow', {
                ...
                _callSelectGoods: function (goods) {
                    if (goods && Ext.isArray(goods)) {
                        ...
                        self.apis.Discount.addDiscountGoods
                            .wait(self, '正在添加促销商品...')
                            .call({id: self._discount['id'], gids: couponGoods}, function () {
                                self.loadGoods();
                            })
                    }
                }
    ...
    onRemoveClick: function () {
        ...
        if (this._discount) {
            ...
            if (datas) {
                ...
                Dialog.batch({
                    message: '确定移除当前促销商品{d}吗？',
                    data: datas,
                    key: 'name',
                    callback: function (btn) {
                        if (btn == Global.YES) {
                            self.apis.Discount.removeDiscountGoods
                                .wait(self, '正在移除促销商品...')
                                .call({id: self._discount['id'], gids: gidList}, function () {
                                    self.refreshStore();
                                });
                        }
                    }
    ...
    loadGoods: function () {
        var did = this._discount['id'];
        var type = this._discount['type'];
        if (type == 1) { // 商品类型
            this.find('add_goods_btn').show();
        }
        var store = this.apis.Discount.getDiscountGoods.createPageStore({did: did});
        this.setStore(store);
        store.load();
    }
});
